...
tree=ast.parse((W/'35_同种子三字频对照/controller.py').read_text(encoding='utf-8-sig'));exec(compile(ast.Module(body=[n for n in tree.body if isinstance(n,ast.FunctionDef) and n.name in ('run','export')],type_ignores=[]),'shared-runner','exec'))
try:
 labels=['小鹤','自然码','拼音加加','智能ABC'];write(O/'status.json',{'状态':'运行中','方案':labels,'seed':seed,'steps':50000})
 def arm(label):
