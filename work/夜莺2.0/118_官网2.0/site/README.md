...
- 首页：发布状态、方案数据（130 组字根、前 1500 字选重 0、字词冲突 55→0）、下载区指向 v2.0 Release 附件（Rime 主力/轻量、各平台码表、啾啾工具箱）、更新日志指向 `releases/v2.5/夜莺2.0升级日志.md`。
- 工具页：从 `releases/v2.5/04_查询与练习/离线工具包/` 复制拆分查询、部件反查、字根练习、完整拆分表、字根图，另发布啾啾工具箱单文件（`tools/toolbox.html`）；每页右下角加返回首页链接。1.0 的编码反查并入拆分查询（输入编码即可），必拆字练习不再提供。
