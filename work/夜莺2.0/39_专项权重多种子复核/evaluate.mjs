...
const P='E:/夜莺2.0/work/夜莺2.0/39_专项权重多种子复核',W=P+'/..';
const paths={"s1_w0": "E:/夜莺2.0/work/夜莺2.0/38_固定200字当量专项试跑/w0_五万步单字表.txt", "s1_w40": "E:/夜莺2.0/work/夜莺2.0/38_固定200字当量专项试跑/w40_五万步单字表.txt", "s1_w120": "E:/夜莺2.0/work/夜莺2.0/38_固定200字当量专项试跑/w120_五万步单字表.txt", "s2_w0": "E:/夜莺2.0/work/夜莺2.0/39_专项权重多种子复核/s2_w0_五万步单字表.txt", "s2_w40": "E:/夜莺2.0/work/夜莺2.0/39_专项权重多种子复核/s2_w40_五万步单字表.txt", "s2_w120": "E:/夜莺2.0/work/夜莺2.0/39_专项权重多种子复核/s2_w120_五万步单字表.txt", "s3_w0": "E:/夜莺2.0/work/夜莺2.0/39_专项权重多种子复核/s3_w0_五万步单字表.txt", "s3_w40": "E:/夜莺2.0/work/夜莺2.0/39_专项权重多种子复核/s3_w40_五万步单字表.txt", "s3_w120": "E:/夜莺2.0/work/夜莺2.0/39_专项权重多种子复核/s3_w120_五万步单字表.txt"};
const out=[];
for(const [name,path] of Object.entries(paths)){
 const items=readFileSync(path,'utf8').replace(/^\uFEFF/,'').trim().split(/\r?\n/).map((l,i)=>{let [w,c]=l.trim().split(/\s+/);return [w,c,i]}).filter(([w,c])=>[...w].length===1&&/^[a-z]{1,4}$/.test(c));
 const ev=quickEvaluateHanzi({items,cmLen:4,selectKeys:" ;'456789"},presetHanziFreq);
 const bands=ev.evaluate.map(b=>{const a=b.items.filter(x=>'code'in x),den=a.reduce((n,x)=>n+x.freq,0);return {档位:`${b.start+1}–${b.end}`,缺字:b.items.length-a.length,码长字数:[1,2,3,4].map(n=>a.filter(x=>x.code.length===n).length),选重字数:a.filter(x=>x.collision>1).length,选重频率:a.filter(x=>x.collision>1).reduce((s,x)=>s+x.freq,0)/den,键均:a.reduce((n,x)=>n+x.keyEq,0)/den,字均:a.reduce((n,x)=>n+x.ziEq,0)/den,键长:a.reduce((n,x)=>n+x.CL,0)/den};});
 const a=ev.evaluate[1].items.filter(x=>'code'in x),den=a.reduce((n,x)=>n+x.freq,0);let totals=[0,0,0,0],lengths=[0,0,0,0];
 const details=a.map(x=>{const c=x.code,e=x.selectKey||'',n=c.length+e.length-1,v=[c.length>=2?calcEq(c.slice(0,2)):0,c.length>=3?calcEq(c.slice(1,3)):0,c.length>=4?calcEq(c.slice(2,4)):0,e?calcEq(c.slice(-1)+e):0].map(z=>z/n);v.forEach((z,i)=>totals[i]+=z*x.freq/den);lengths[c.length-1]++;return {字:x.wd,码:c,选键:e,字频:x.freq,当量:x.keyEq/x.freq,贡献:v,键长:x.CL/x.freq};});
 if(Math.abs(totals.reduce((a,b)=>a+b)-bands[1].键均)>1e-9)throw Error('sum');
 out.push({方案:name,有效行:items.length,分档:bands,全字明细:ev.evaluate,成本:totals,码长字数:lengths,逐字:details});
}
writeFileSync(P+'/测评明细.json',JSON.stringify(out,null,2));console.log('All native box evaluations complete');
