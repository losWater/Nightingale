...
tree=ast.parse((W/'35_同种子三字频对照/controller.py').read_text(encoding='utf-8-sig'));source=ast.unparse(ast.Module(body=[n for n in tree.body if isinstance(n,ast.FunctionDef) and n.name in ('run','export')],type_ignores=[]));source=source.replace("NIGHTINGALE_TRIAL_SEED=str(seed)", "NIGHTINGALE_TRIAL_SEED=str(read(d.parent/'job.json')['seed']), NIGHTINGALE_TARGET_DIR=str(O), NIGHTINGALE_TARGET_WEIGHT=str(read(d.parent/'job.json')['weight'])");exec(source)
try:
 labels=[j['id'] for j in read(O/'jobs.json')];write(O/'status.json',{'状态':'运行中','方案':labels,'seed':seed,'steps':50000})
 def arm(label):
