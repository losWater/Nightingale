...
def main():
 ...
 try:
  ...
  prepare_first();prepare_second();cards=read(ROOT/'cards.json');promoted=[];seen=set()
  for group in range(1,SET['groups']+1):
   ...
   for r in ranked:
    ...
    assert read(ROOT/'jobs'/r['id']/'optimized.json')['primary_guard_pass']
    if r['layout_hash'] not in seen:chosen.append(r);seen.add(r['layout_hash'])
    if len(chosen)==SET['promote']:break
   write(ROOT/f'group_{group:02d}_selection.json',{'ranked':[r['id'] for r in ranked],'selected':[r['id'] for r in chosen],'rule':'1643字主音有首选，综合分降序、全赛去重；不足4名停止，不放宽门槛'})
   extra_no=0
   while len(chosen)<SET['promote']:
    extra_no+=1
    if extra_no>32:raise RuntimeError(f'组{group}补跑32次仍不足4个独立布局')
    card={'id':f'g{group:02d}_supplement_{extra_no:02d}','group':group,'kind':'random' if extra_no%2 else 'large','seed':SET['seed']+100000+group*100+extra_no}
    if not any(c['id']==card['id'] for c in cards):cards.append(card);write(ROOT/'cards.json',cards)
    rr=batch([card],1)[0];ranked.append(rr)
    if rr['layout_hash'] not in seen:chosen.append(rr);seen.add(rr['layout_hash'])
   ranked.sort(key=lambda r:(-r['score']['total'],r['id']))
   write(ROOT/f'group_{group:02d}_selection.json',{'ranked':[r['id'] for r in ranked],'selected':[r['id'] for r in chosen],'supplementary_runs':extra_no,'rule':'1643主音硬保护；每组4个全赛唯一布局；不足时记录额外补跑，不降低标准'})
  ...
  finish(finals,allrank);status('complete',first_round_completed=len(list((ROOT/'jobs').glob('*/round1.json'))),second_round_completed=128,finalists=[r['id'] for r in finals],report=str(ROOT/'最终候选报告.html'))
 except Exception:
