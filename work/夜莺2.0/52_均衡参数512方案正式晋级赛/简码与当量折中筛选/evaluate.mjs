...
const P='E:/夜莺2.0/work/夜莺2.0/52_均衡参数512方案正式晋级赛';
function evaluate(id,txt,custom=false){
 const items=txt.replace(/^\uFEFF/,'').trim().split(/\r?\n/).map((l,i)=>{if(custom){const [c,t]=l.split('=');return [t.split(',')[1],c,i]}let [w,c]=l.trim().split(/\s+/);return [w,c,i]}).filter(([w,c])=>[...w].length===1&&/^[a-z]{1,4}$/.test(c));
 const ev=quickEvaluateHanzi({items,cmLen:4,selectKeys:" ;'456789"},presetHanziFreq);
 const aggregate=a=>{const den=a.reduce((s,x)=>s+x.freq,0);return {count:a.length,short:a.filter(x=>x.code.length<=3).length,shortFirst:a.filter(x=>x.code.length<=3&&x.collision===1).length,eq:a.reduce((s,x)=>s+x.keyEq,0)/den,select:a.filter(x=>x.collision>1).length}};
 const bands=ev.evaluate.map(b=>aggregate(b.items.filter(x=>'code'in x)));
 const top=aggregate(ev.evaluate.slice(0,3).flatMap(b=>b.items.filter(x=>'code'in x)));
 return {id,bands,top};
}
const baseline=evaluate('夜莺1.0',readFileSync('E:/夜莺2.0/releases/v1.0/01_正式码表/夜莺码v1.0单字版.txt','utf8'));
const out=[];
for(const id of readdirSync(P+'/jobs').sort()){
 const file=P+'/round1/results/'+id+'/纯单字码表.txt';
 out.push(evaluate(id,readFileSync(file,'utf8'),true));
 if(out.length%64===0)console.log('evaluated',out.length);
}
writeFileSync(P+'/简码与当量折中筛选/全部测评.json',JSON.stringify({baseline,results:out},null,2));
console.log('BASE',JSON.stringify(baseline));console.log('PASS',out.filter(x=>x.top.short>=baseline.top.short-50).length);
console.log(JSON.stringify(out.filter(x=>x.top.short>=baseline.top.short-50).sort((a,b)=>a.bands[1].eq-b.bands[1].eq).slice(0,10)));
