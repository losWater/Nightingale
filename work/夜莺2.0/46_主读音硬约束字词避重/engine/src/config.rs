...
pub struct 层级权重 {
    ...
    /// 忽略已有一简或二简的字之后统计的全码选重。
    pub effective_duplication: Option<f64>,
    /// 有效全码候选位置的平方惩罚。
    pub effective_duplication_squared: Option<f64>,
    pub levels: Option<Vec<码长权重>>,
    pub fingering: Option<指法权重>,
    /// 按对象频率加权，并按该层加权键对总量归一化的差指法。
    pub weighted_fingering: Option<指法权重>,
    /// 仅统计第二个音码键到第一个型码键的二键当量。
...
pub struct 交叉码位权重 {
    ...
    pub hard: bool,
    #[serde(default)]
    pub hard_character_top: Option<usize>,
}
...
pub struct 交叉碰撞配置 {
    ...
    pub hard_character_top: usize,
    pub character_tiers: Vec<交叉字符层>,
    pub targets: IndexMap<String, 交叉码位权重>,
}
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct 辅助码配置 {
    /// 辅码后加权候选名次负担在总目标函数中的权重。
    pub weight: f64,
    pub records: Vec<辅助码词配置>,
}
...
pub struct 目标配置 {
    ...
    pub character_word_collision: Option<交叉碰撞配置>,
    pub auxiliary_two_char: Option<辅助码配置>,
