...
pub struct 层级指标 {
    ...
    pub effective_duplication: Option<u64>,
    pub effective_duplication_squared: Option<u64>,
    pub levels: Option<Vec<LevelMetricUniform>>,
    pub fingering: Option<FingeringMetricUniform>,
    pub weighted_fingering: Option<FingeringMetric>,
    pub phonetic_shape_transition_equivalence: Option<f64>,
}
...
impl Display for 层级指标 {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        if let Some(value) = self.effective_duplication {
            f.write_str(&format!("{specifier}有效全码选重：{value}；"))?;
        }
        if let Some(value) = self.effective_duplication_squared {
            f.write_str(&format!("{specifier}有效全码选重平方：{value}；"))?;
        }
        if let Some(fingering) = &self.weighted_fingering {
            for (index, frequency) in fingering.iter().enumerate() {
                if let Some(frequency) = frequency {
                    f.write_str(&format!("加权{}：{frequency:.4}；", 指法标记名称[index]))?;
                }
            }
        }
        if let Some(equivalence) = self.phonetic_shape_transition_equivalence {
            f.write_str(&format!("音型过渡当量：{equivalence:.4}；"))?;
        }
        Ok(())
    }
}
...
pub struct 分组指标 {
    ...
    pub effective_duplication: Option<f64>,
...
impl Display for 分组指标 {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        if let Some(value) = self.effective_duplication {
            f.write_str(&format!("有效全码选重率：{:.4}%；", value * 100.0))?;
        }
        if let Some(equivalence) = self.phonetic_shape_transition_equivalence {
            f.write_str(&format!("音型过渡当量：{equivalence:.4}；"))?;
        }
...
pub struct 默认指标 {
    ...
    pub character_word_collision: Option<交叉碰撞指标>,
    pub auxiliary_two_char: Option<辅助码指标>,
...
pub struct 辅助码指标 {
    ...
    pub weighted_first_choice_rate: f64,
    pub rank_burden: f64,
}
...
impl Display for 默认指标 {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        if let Some(cross) = &self.character_word_collision {
            f.write_str(&format!(
                "字词交叉［硬碰撞：{}；软碰撞当量：{:.4}；］\n",
                cross.hard, cross.soft
            ))?;
        }
        if let Some(auxiliary) = &self.auxiliary_two_char {
            f.write_str(&format!(
                "五码定二字［同音词：{}；辅码首选：{}；加权首选率：{:.2}%；加权选重负担：{:.4}；］\n",
                auxiliary.ambiguous,
                auxiliary.first_choice,
                auxiliary.weighted_first_choice_rate * 100.0,
                auxiliary.rank_burden
            ))?;
        }
        Ok(())
    }
