...
impl 退火方法 {
    ...
    fn solve_with<O: 目标函数, F: 变异<决策 = O::决策>, C: 上下文<决策 = O::决策>>(
        ...
        assert!(目标函数.可行(), "initial layout fails primary reading hard constraint");
        ...
        for 步骤 in 起始步数..总步数 {
            ...
            if feasible && (改进 < 0.0 || (crate::trial_rng::random_f64() < (-改进 / 温度).exp())) {
                accepted[phase] += 1;
        ...
        eprintln!("PRIMARY_INFEASIBLE_REJECTED {}", infeasible);
        eprintln!("TRIAL_ACCEPT {:?} / {:?}", accepted, attempted);
        let 增量最优指标 = 最优指标.clone();
        最优指标 = 目标函数.计算(&最优决策, &None);
        assert!(目标函数.可行(), "final layout fails primary reading hard constraint");
