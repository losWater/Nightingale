...
for nr, seq in {'甬字底': ['冂', '举字底'], '敖左': ['龶', '折', '丿'], '卸左': ['𠂉', '正'], '車': ['一', '日', '十']}.items():   # 回归查出的 2.0 合并根
    if nr in norm: merges.append(([x for y in seq for x in expand(nid(y))], nid(nr)))
...
json.dump({'回归不同': diffs, '拆分': ok, '对不上': bad, '拟码': codes}, open(H + '/继承试算5.json', 'w', encoding='utf-8'), ensure_ascii=False, indent=0)
