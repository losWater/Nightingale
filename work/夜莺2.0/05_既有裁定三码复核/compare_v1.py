...
labels={n['og'](n['rid'](x)):x for x in ['亻','彳','卧人','行']};b=defaultdict(lambda:defaultdict(list))
...
notes='对照夜莺1.0正式字词码表的三码首选（包含词的真实候选顺序），按对应音节核对。只看亻、彳、卧人、行四组合并的假设，该方案仍待定，不启用累计合并。直接风险为不同原根组各有实际三简，合并后同一音节只剩一个自然三码首选位置。谁让位尚未裁决；表按当前分音频次排序，不以此替换1.0人工选择。未重新安排一二简、实际键位或容错码。单字形态分析沿用当前2.0试算，完整候选组含其他变化；本报告隔离四组合并新增的跨组竞争。'
...
(HERE/'对照1.0实际三简.json').write_text(json.dumps(data,ensure_ascii=False,indent=2),encoding='utf-8')
