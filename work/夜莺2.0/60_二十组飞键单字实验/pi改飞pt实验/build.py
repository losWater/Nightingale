...
pairs=dict(x.split(':') for x in 'cs:cl dc:dl gr:gp gv:gi jm:jz lp:lf mj:mr mu:mx nj:nz pi:pt pk:px po:ps qx:qo tv:tl uj:us vg:vk xq:xo yj:yf yu:yg zs:zl'.split())
...
(P/'普通单字表_pi飞pt实验.txt').write_text(''.join(c+'\t'+k+'\n' for k,cs in sorted(dest.items()) for c in cs),encoding='utf-8-sig')
(P/'普通单字表_原版对照.txt').write_bytes(raw)
meta={'仅实验':True,'来源':str(b),'来源SHA256':hashlib.sha256(raw).hexdigest(),'飞键规则':pairs,'条目数':len(old),'汉字数':len({c for c,k in old}),'变化条目':len(changes),'涉及字数':len({r['字'] for r in changes}),'新增合码':coll,'规则':'整体替换二三四码音部，原音码不作为兼容码保留；一简、形部不变。飞入字优先，原占位字退后；飞入字之间保持原候选顺序；无重排简码，无退火，不实装。'}
