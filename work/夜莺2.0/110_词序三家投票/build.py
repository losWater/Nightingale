...
无多数则剩余保持鲸凉鹤原序（待定，留清单）。字的位置不动，人工指定的码位不动。2026-09-16 扩到三简位的三字词（103 层）：人工裁定/人工配置来的词钉在原位不参与投票，其余同样投票。输入 104 最终表，输出本目录最终表四种格式，106 改从这里取。"""
...
def vote(words):
    ...
    for name, score in SCORERS:
        ...
        if len(sc) < 2: continue   # 2026-09-16 修：该家至少收了两个词、真正比较过才有投票权；只收一个不算票（否则缺席被当成反对，口语短语吃亏）
...
pinned = {(x['码'], x['词']) for x in json.load(open(B + '/103_三字词三码/生成报告.json', encoding='utf-8'))['明细'] if x['来由'] != '鲸凉鹤原序'}   # 103 层人工项钉在原位
changed = []; pending = []; nvote = 0
for c, blk in blocks.items():
    if len(c) not in (3, 4) or c in man: continue
    slots = [i for i, w in enumerate(blk) if len(w) > 1 and (c, w) not in pinned]; words = [blk[i] for i in slots]
...
for c in b104:
    ...
    if c in man or len(c) not in (3, 4): assert a == b, c
    if len(c) == 3: assert all(x == y for i, (x, y) in enumerate(zip(a, b)) if (c, x) in pinned), c
firsts = sum(1 for x in changed if x['原序'][0] != x['新序'][0]); n3 = sum(1 for x in changed if len(x['码']) == 3)
rep = {'时间': datetime.datetime.now().isoformat(timespec='seconds'), '输入': '104/夜莺2.0最终表_普通格式.txt', '投票码位': nvote, '次序有变的码位': len(changed), '首选有变的码位': firsts, '其中三码位有变': n3, '待定码位': len(pending), '输出': out, '变化明细': changed, '待定明细': pending}
json.dump(rep, open(H + '/生成报告.json', 'w', encoding='utf-8'), ensure_ascii=False, indent=1)
print('投票码位 %d；次序有变 %d（首选有变 %d，三码位有变 %d）；待定 %d；自检通过' % (nvote, len(changed), firsts, n3, len(pending)))
