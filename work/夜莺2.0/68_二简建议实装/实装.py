...
conf=read(P/'裁定配置.json');targets=conf['码位安排'];ptr=T/'二简人工复核/当前裁定基线.json';previous=read(ptr);old=Path(previous['目录']);new=old.parent/'第二十批_二简建议明确项'
...
jout(V/'码位人工指定.json',targets)
module='''from pathlib import Path
import json
def apply(combined,short_only=False):
 ...
 return combined
'''
write(V/'apply_explicit_candidates.py',module)
f=V/'build.py';s=f.read_text(encoding='utf-8');s=s.replace("cap=runpy.run_path(str(P/'cap_short_words.py'))['apply_cap']","combined=runpy.run_path(str(P/'apply_explicit_candidates.py'))['apply'](combined,short_only=True)\ncap=runpy.run_path(str(P/'cap_short_words.py'))['apply_cap']")
needle="combined,full_summary=runpy.run_path(str(P/'full_code_order.py'))['reorder'](combined,source,restore,merges)"
s=s.replace(needle,needle+"\ncombined=runpy.run_path(str(P/'apply_explicit_candidates.py'))['apply'](combined)")
s=s.replace("summary['全码排序']=full_summary","summary['人工指定码位']=json.loads((P/'码位人工指定.json').read_text(encoding='utf-8'))\nsummary['全码排序']=full_summary")
s=s.replace("summary['验证']='单字码位不变；原有全码保留；同码单字相对顺序不变；至多一个词先于单字且单字连续；所有撤码可追溯；二字词撤简码前有全码；无重复词码；暂缓与人工例外之外至多三选。'","summary['验证']='单字码位与当前基线一致；原有全码保留；人工指定码位优先（含qtxb确信在碏前）；其余遵循既有让位及三选规则；所有撤码可追溯；无重复词码。'")
write(f,s)
