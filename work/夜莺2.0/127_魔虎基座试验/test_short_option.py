...
T = 'E:/ymt'; H = os.path.dirname(os.path.abspath(__file__)); C = T + '/mohu_flypy.custom.yaml'
...
off = run()
txt = open(H + '/夜莺魔虎试验版/简词进整句_启用方法.txt', encoding='utf-8').read(); open(C, 'w', encoding='utf-8').write(txt[txt.index('patch:'):])
try: on = run()
finally: os.remove(C)
