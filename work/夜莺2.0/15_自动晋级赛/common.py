...
def prepare_second():
 ...
 excluded={r['汉字'] for r in read(F/'round1/固定一万句.json')};charset={r['字'] for r in read(F/'字音基准.json')};rng=random.Random(SET['second_seed']);allrows=[]
 ...
 for tag,fn,name in sources:
  pool=[];seen=set();n=0
  for original,text in fn(F/'raw'/name,200000):
