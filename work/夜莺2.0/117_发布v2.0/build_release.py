...
from 版本 import VER, DATE, TAG, RELEASE_DIR as R   # 版本号唯一出处：00_维护/版本.py
sha = lambda p: hashlib.sha256(open(p, 'rb').read()).hexdigest()
plan = {   # 目标相对路径 -> 源
 # 发布的是综合表与普通单字表；字词表、纯单字表、符号表是原本，不发布（2026-09-17 三表架构）
 '01_正式码表/夜莺2.0综合表.txt': X + '/夜莺2.0_字词表与输入法/普通字词表/夜莺2.0_综合表_普通.txt',
 '01_正式码表/夜莺2.0综合表_码前.txt': X + '/夜莺2.0_字词表与输入法/普通字词表/夜莺2.0_综合表_码前.txt',
 '01_正式码表/夜莺2.0普通单字表.txt': X + '/夜莺2.0_字词表与输入法/普通字词表/夜莺2.0_普通单字表_普通.txt',
 '01_正式码表/夜莺2.0普通单字表_码前.txt': X + '/夜莺2.0_字词表与输入法/普通字词表/夜莺2.0_普通单字表_码前.txt',
...
big = {   # 大包：本地 发布包/ + GitHub Release 附件
 ...
 f'Nightingale-Rime-{VER}-main-{DATE}.zip': (W + '/127_魔虎基座试验/夜莺2.0_Rime主力版.zip', '夜莺2.0_Rime主力版_整句含模型_电脑用.zip', '夜莺主力 · Rime 主力版（基于魔虎 rime-mohu 制作，夜莺只换了码表词库；整句含模型，电脑用）'),
 f'Nightingale-Rime-{VER}-light-{DATE}.zip': (X + '/夜莺2.0_Rime轻量版_含快符.zip', '夜莺2.0_Rime轻量版_无模型_小体积.zip', 'Rime 轻量版（Rime 原生整句，无模型，体积小）'),
 f'Nightingale-Rime-{VER}-mobile-{DATE}.zip': (X + '/夜莺2.0_Rime手机版_万象模型_含快符.zip', '夜莺2.0_Rime手机版_同文与仓_万象模型.zip', 'Rime 手机版（同文／仓输入法，万象语法模型）'),
 f'Nightingale-{VER}-tables-{DATE}.zip': (X + '/夜莺2.0_字词表与输入法_含快符.zip', '夜莺2.0_码表与挂接包_手心搜狗冰凌Bime.zip', '码表与挂接包（手心、搜狗、冰凌、Bime、纯码表）'),
 f'Nightingale-Toolbox-{VER}-{DATE}.html': (U + '/夜莺啾啾工具箱.html', '夜莺啾啾工具箱_拆分查询与字根练习.html', '啾啾工具箱（单个网页：拆分查询、部件反查、字根练习）'),
 f'Nightingale-Toolbox-offline-{VER}-{DATE}.zip': (U + '/夜莺2.0离线工具包.zip', '夜莺2.0_离线工具包.zip', '离线工具包（工具箱各页面的离线版）'),
}
...
- 主力版「夜莺主力」：**主体是魔虎（rime-mohu，作者 晴，https://github.com/fcxxxz/rime-mohu ，GPL v3）**，整句引擎、V5 模型与全部功能脚本是魔虎作者的成果，夜莺换上了自己的码表、词库、辅码、拆分；文件改用 yeying 名字以便与魔虎原版共存，出处逐文件注明（包内有声明与改动清单）。约 450MB，不入 git：见 GitHub Release ' + TAG + ' 附件 `Nightingale-Rime-{VER}-main-{DATE}.zip`；本地留存在 `发布包/`（已 gitignore，中文文件名）。
- 手机版（轻量版 + 万象 LTS 语法模型 `wanxiang-lts-zh-hans.gram`，约 360MB，供同文/仓输入法等手机 Rime）：Release 附件 `Nightingale-Rime-{VER}-mobile-{DATE}.zip`；魔虎 V5 引擎只有 Windows 版，手机用不了。
- 轻量版与手机版：Ctrl+数字 钉选、二字自动造词、Ctrl+Enter 主动造词；主力版用魔虎自带的置顶与加词。三版共有：反引号双拼/波浪号全拼反查、F2、40 条快符。
- 校验：`发布包/校验值SHA256.txt`。
""")
open(R + '/README.md', 'w', encoding='utf-8').write(f"""# 夜莺 {VER} 发布目录（{DATE}）
...
校验值见 `发布清单.json`。
""")
json.dump({'version': VER, 'date': DATE, 'status': 'release', 'files': manifest, 'release_assets': assets,
           'final_table_entries': sum(1 for _ in open(W + '/00_维护/主表/夜莺2.0字词表.txt', encoding='utf-8')), 'chars': 15496, 'note': '大包不入 git，见 GitHub Release ' + TAG + ' 附件与本地 02_输入法挂接/rime/发布包/'},
...
print('releases/v2.5：%d 个文件 %.1f MB（入 git）；发布包附件 %d 个 %.1f MB（不入 git）' % (len(manifest), tot / 1048576, len(assets), sum(v['bytes'] for v in assets.values()) / 1048576))
