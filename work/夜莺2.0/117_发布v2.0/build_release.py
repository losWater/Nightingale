...
- 主力版「夜莺主力」：**主体是魔虎（rime-mohu，作者 晴，https://github.com/fcxxxz/rime-mohu ，GPL v3）**，整句引擎、V5 模型与全部功能脚本是魔虎作者的成果，夜莺换上了自己的码表、词库、辅码、拆分；文件改用 yeying 名字以便与魔虎原版共存，出处逐文件注明（包内有声明与改动清单）。约 450MB，不入 git：见 GitHub Release v2.0 附件 `Nightingale-Rime-2.0-main-{DATE}.zip`；本地留存在 `发布包/`（已 gitignore，中文文件名）。
