...
s='<!doctype html><html lang="zh-CN"><meta charset="utf-8"><title>主读音准入与字词避重</title><style>body{font:16px/1.7 system-ui;background:#f4f7fb;color:#234;margin:28px}table{border-collapse:collapse;background:white}td,th{border:1px solid #ccd;padding:8px}</style><h1>主读音准入＋字词避重</h1><p>当量60/60/40/25/15，恢复50%跨排惩罚；只把字词软碰撞系数0.1改成0.2或0.4。同起点与种子202609121702，各五万步。原系数复用43号该种子结果。</p><p>用户确认1643字按常用主读音保护：简码或全码至少一处首选。最终输出准入是硬门槛，搜索仍用原软损失，允许探索未达标状态；不能把不达标结果推荐为最终方案。选主音依据自然分音频率，不用优化降权值。主音检查为单字码表口径，字词冲突另外报告。</p><p>单种子试探，不表示参数稳定最优。后两档各缺2、38字；同一盒子频率独立测评。</p>'
...
s+='<h2>分段当量</h2>'
s+='<p><strong>结论：本轮两组加强字词避重的结果都未通过1643字主读音准入，不能替换现有候选。2倍组诗不达标；4倍组述、映、肤不达标。4倍组字词软碰撞虽从36.802411降至12.029436，但该收益不能抵消已确认的主音零重码要求。原系数组通过，保留为当前对照。</strong></p><p>下一步需把主音要求纳入搜索内的可行性约束或强惩罚并保存可行最优，再进行字词权重试验；本轮只实现最终输出准入检查，没有声称搜索内已强制零重码。</p>'
...
s+='<details><summary>全部原生指标及相对原系数的变化</summary><p>指法数组的各项按引擎定义；字段保留计算，两组只加强字词软碰撞惩罚系数。覆盖率等指标增加为改善，不能把所有负差都当改善。</p>'
