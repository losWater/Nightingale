...
const P='E:/夜莺2.0/work/夜莺2.0/37_简单鹤同口径成本对照',W=P+'/..';
const paths={'简单鹤':P+'/简单鹤V1.0纯单版.txt','夜莺1.0':'E:/夜莺2.0/releases/v1.0/01_正式码表/夜莺码v1.0单字版.txt','夜莺2.0小鹤':W+'/36_双拼音部替换对照/小鹤_五万步单字表.txt','夜莺2.0ABC':W+'/36_双拼音部替换对照/智能ABC_五万步单字表.txt'};
const out=[];
for(const [name,path] of Object.entries(paths)){
 ...
 const bands=ev.evaluate.map(b=>{const a=b.items.filter(x=>'code'in x),den=a.reduce((n,x)=>n+x.freq,0);return {档位:`${b.start+1}–${b.end}`,缺字:b.items.length-a.length,键均:a.reduce((n,x)=>n+x.keyEq,0)/den,字均:a.reduce((n,x)=>n+x.ziEq,0)/den,键长:a.reduce((n,x)=>n+x.CL,0)/den};});
 ...
 out.push({方案:name,有效行:items.length,分档:bands,成本:totals,码长字数:lengths,逐字:details});
}
const j=out[0],n=out[2],den=n.逐字.reduce((s,x)=>s+x.字频,0);
const diff=n.逐字.map(x=>{const y=j.逐字.find(z=>z.字===x.字);if(!y)throw Error('missing '+x.字);return {字:x.字,夜莺码:x.码,简单鹤码:y.码,夜莺当量:x.当量,简单鹤当量:y.当量,加权差:(x.当量-y.当量)*x.字频/den,分项差:x.贡献.map((v,i)=>(v-y.贡献[i])*x.字频/den),音码一致:x.码.slice(0,2)===y.码.slice(0,2)}}).sort((a,b)=>b.加权差-a.加权差);
writeFileSync(P+'/测评明细.json',JSON.stringify(out,null,2));writeFileSync(P+'/逐字差距.json',JSON.stringify(diff,null,2));
console.log(JSON.stringify(out.map(({逐字,...r})=>r)));console.log(JSON.stringify(diff.slice(0,20)));
