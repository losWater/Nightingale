...
def arm(j):
 ...
 cfg=initial
 if bad:
  repair=json.loads(json.dumps(initial));repair['optimization']['metaheuristic']['parameters']={'t_max':3000,'t_min':30,'steps':50000}
  ...
  cfg=mapping(out);cfg['generated_mapping_space']=initial['generated_mapping_space']
  ...
  assert abs(read(out/'metric.json')['score']-read(verify/'metric.json')['score'])<1e-6
