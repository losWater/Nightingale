...
impl 退火方法 {
    pub fn 优化<O: 目标函数, F: 变异<决策 = O::决策>, C: 上下文<决策 = O::决策>>(
        &self,
        初始决策: &O::决策,
        目标函数: &mut O,
        操作: &mut F,
        ...
        assert!(std::env::var("NIGHTINGALE_PRIMARY_GUARD").is_err() || self.parameters.is_some(), "primary guard requires explicit annealing schedule");
        let 降温时间表 = self
            .parameters
    ...
    /// 退火算法求决策的主函数
    fn solve_with<O: 目标函数, F: 变异<决策 = O::决策>, C: 上下文<决策 = O::决策>>(
        &self,
        初始决策: &O::决策,
        目标函数: &mut O,
        操作: &mut F,
        ...
        let repair = std::env::var_os("NIGHTINGALE_REPAIR").is_some();
        assert!(!repair || std::env::var_os("NIGHTINGALE_PRIMARY_GUARD").is_some(), "repair requires guard manifest");
        let mut 最优决策 = 初始决策.clone();
        let mut 最优指标 = 目标函数.计算(&最优决策, &None);
        assert!(repair || 目标函数.可行(), "initial layout fails primary reading hard constraint");
        let mut 当前决策 = 最优决策.clone();
        ...
        let mut executed = 起始步数;
        for 步骤 in 起始步数..总步数 {
            if repair && 目标函数.可行() && 上一个变化.is_none() { break; }
            executed = 步骤 + 1;
            let phase = (步骤 * 3 / 总步数).min(2);
            attempted[phase] += 1;
            ...
            let feasible = 目标函数.可行();
            if !feasible { infeasible += 1; }
            // Keep the normal rejected-move bookkeeping: caches currently describe the trial.
            if (repair && feasible) || ((repair || feasible) && (改进 < 0.0 || (crate::trial_rng::random_f64() < (-改进 / 温度).exp()))) {
                accepted[phase] += 1;
        ...
        // 最后完整重算最优解：既让输出配置和码表一致，也以完整重算结果覆盖
        // 增量搜索期间保存的指标，避免报告继续携带可能的缓存累计误差。
        eprintln!("STAGE_REPAIR {} EXECUTED {}", repair, executed);
        eprintln!("PRIMARY_INFEASIBLE_REJECTED {}", if repair {0} else {infeasible});
        eprintln!("TRIAL_ACCEPT {:?} / {:?}", accepted, attempted);
        let 增量最优指标 = 最优指标.clone();
        最优指标 = 目标函数.计算(&最优决策, &None);
        assert!(repair || 目标函数.可行(), "final layout fails primary reading hard constraint");
        eprintln!("TRIAL_CACHE_DELTA {}", (增量最优指标.1 - 最优指标.1).abs());
        if format!("{}", 增量最优指标.0) != format!("{}", 最优指标.0) {
            eprintln!(
                "警告：最优解的增量指标与完整重算不一致。\n增量：{}\n完整：{}",
                增量最优指标.0, 最优指标.0
            );
    ...
    fn trial_run<O: 目标函数, F: 变异<决策 = O::决策>>(
        &self,
        目标函数: &mut O,
        操作: &mut F,
        from: O::决策,
