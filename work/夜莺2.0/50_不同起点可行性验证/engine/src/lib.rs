...
pub struct 原始可编码对象 {
    ...
    pub 频率: u64,
    /// 可选的优化内部顺序。用于自定义保护层和简码竞争顺序，同时保留真实频率参与加权。
    #[serde(default)]
    pub 排序序号: Option<usize>,
...
impl 棱镜 {
    ...
    pub fn 预处理元素序列(
        ...
        for (i, 码位) in 原始元素序列.iter().enumerate() {
            let 元素位 = match 码位 {
                ...
                广义码位::Ascii(k) => {
                    if let Some(&键) = self.键转数字.get(k) {
                        // 把按键转换为元素位，按键 0 对应元素位 0，按键 1 对应元素位 1，以此类推
                        键 as usize
                    } else {
                        return Err(format!(
                            "编码对象「{词}」包含的按键「{k}」无法在键盘映射中找到"
                        )
                        .into());
                    }
                }
    ...
    pub fn 预处理词列表(
        ...
        // 若资产显式给出排序序号，则优先采用；未给出的旧资产仍按频率降序，保持兼容。
        // 原始顺序是稳定兜底，避免同频对象在不同平台上发生不必要漂移。
        let has_explicit_order = 词列表.iter().any(|x| x.排序序号.is_some());
        if has_explicit_order {
            词列表.sort_by_key(|x| (x.排序序号.unwrap_or(usize::MAX), x.原始顺序));
        } else {
            词列表.sort_by_key(|x| (Reverse(x.频率), x.原始顺序));
        }
        Ok(词列表)
    }
