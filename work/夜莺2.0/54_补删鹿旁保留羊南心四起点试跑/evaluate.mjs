...
const P='E:/夜莺2.0/work/夜莺2.0/54_补删鹿旁保留羊南心四起点试跑';const cards=JSON.parse(readFileSync(P+'/cards.json','utf8'));
const paths={g06_large_01:'E:/夜莺2.0/work/夜莺2.0/52_均衡参数512方案正式晋级赛/finalists/01_g06_large_01/普通纯单字表.txt'};
for(const c of cards)paths[c.id]=P+'/普通码表/'+c.id+'.txt';const out=[];
for(const [id,path] of Object.entries(paths)){const items=readFileSync(path,'utf8').replace(/^\uFEFF/,'').trim().split(/\r?\n/).map((l,i)=>[...l.split(/\s+/),i]);const ev=quickEvaluateHanzi({items,cmLen:4,selectKeys:" ;'456789"},presetHanziFreq);const bands=ev.evaluate.map(b=>{const a=b.items.filter(x=>'code'in x),den=a.reduce((s,x)=>s+x.freq,0);return {count:a.length,short:a.filter(x=>x.code.length<=3).length,eq:a.reduce((s,x)=>s+x.keyEq,0)/den}});const a=ev.evaluate.slice(0,3).flatMap(b=>b.items.filter(x=>'code'in x));out.push({id,bands,short:a.filter(x=>x.code.length<=3).length,eq1500:a.reduce((s,x)=>s+x.keyEq,0)/a.reduce((s,x)=>s+x.freq,0)})}writeFileSync(P+'/盒子分档对照.json',JSON.stringify(out,null,2));
