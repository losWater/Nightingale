...
const active=JSON.parse(readFileSync(dir+'/wenjiao-selected.json','utf8'));const targets=JSON.parse(readFileSync(dir+'/review-deletions.json','utf8'));const out:any={};for(const key of ['base',...Array.from({length:14},(_,i)=>'DEL-'+String(i+3).padStart(3,'0'))]){const mapping={...cfg.form.mapping,龟:'a',用:'a','\ue08c':'a'};const rep=new Map<string,string[]>();for(const t of [...active,...(key==='base'?[]:[targets[key]])]){for(const id of t.ids)delete mapping[id];rep.set(t.id,t.replacement)}delete mapping['角'];rep.set('角',[input.norm['⺈'],'用']);const custom=Object.fromEntries(Object.entries(cfg.analysis?.customize??{}).map(([k,v])=>[k,(v as string[]).flatMap(x=>rep.get(norm(x))??[x])]));delete custom['用'];delete custom['角'];out[key]=analyze(mapping,custom,Object.keys(input.expected));console.log(key)}writeFileSync(dir+'/fourteen-current-raw.json',JSON.stringify(out));