...
const selected=JSON.parse(readFileSync(dir+'/selected-14.json','utf8'));
const mapping={...cfg.form.mapping};
const replacements=new Map<string,string[]>();
for(const t of selected){for(const id of t.ids)delete mapping[id];replacements.set(t.id,t.replacement)}
const custom=Object.fromEntries(Object.entries(cfg.analysis?.customize??{}).map(([k,v])=>[k,(v as string[]).flatMap(x=>replacements.get(norm(x))??[x])]));
const rows=analyze(mapping,custom,Object.keys(input.expected));
for(const [c,seq]of Object.entries(rows))if(seq.some(s=>replacements.has(s)))throw new Error('deleted root remains '+c);
writeFileSync(dir+'/joint-14-raw.json',JSON.stringify(rows));
console.log('joint complete',Object.keys(rows).length);
