...
const custom={...cfg.analysis?.customize};delete custom['万'];const rows=analyze({...cfg.form.mapping,万:'a'},custom,Object.keys(input.expected));writeFileSync(dir+'/add-wan-raw.json',JSON.stringify(rows));console.log(Object.keys(rows).length);