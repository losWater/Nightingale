...
r=json.loads((P/'delete-li-current-raw.json').read_text(encoding='utf-8'))
assert r['base']==json.loads((P/'delete-ju-current-raw.json').read_text(encoding='utf-8'))['joint']
rows=dict(base)
for ch,seq in r['joint'].items():
 ...
 for tag,i,j,a,b in reversed(difflib.SequenceMatcher(None,old,seq,autojunk=False).get_opcodes()):
  ...
  updated[i:j]=seq[a:b]
 rows[ch]=updated
assert all('里' not in seq for seq in rows.values())
a=audit('删除里，拆甲二',base,rows,G,G);a['id']='DEL-029'
a['新增完整频次']=[{'音节':s,'字1':x,'频次1':n['freq'].get((s,x)),'字2':y,'频次2':n['freq'].get((s,y))} for s,x,y in a['新增完整明细']]
(P/'里删根最新试算.json').write_text(json.dumps(a,ensure_ascii=False,indent=2),encoding='utf-8')
for k in ['新增首根字对','三码位净减少','新增完整重码','消除完整重码','新增完整频次','1.0三简风险','拆分变化']:print(k,a[k])
md=['# 里删根试算','','基于已加入生亡乍不的当前累计方案；仅试算，未裁定。里拆甲二。','','| 字 | 原拆分 | 新拆分 |','|---|---|---|']
md+=['| '+x['字']+' | '+x['原拆分']+' | '+x['新拆分']+' |' for x in a['拆分变化']]
md+=['',str({k:a[k] for k in ['新增首根字对','三码位净减少','新增完整频次','1.0三简风险']})]
(P/'里删根最新试算.md').write_text('\n'.join(md),encoding='utf-8')
