...
const tasks=JSON.parse(readFileSync(dir+'/review-deletions.json','utf8'));const active=JSON.parse(readFileSync(dir+'/wenjiao-selected.json','utf8'));const cases:any={current:active};for(const [id,t]of Object.entries(tasks) as any){cases[id]=active.some((x:any)=>x.id===t.id)?active.filter((x:any)=>x.id!==t.id):[...active,t]};const out:any={};for(const [id,sel]of Object.entries(cases) as any){const mapping={...cfg.form.mapping,龟:'a'};const rep=new Map<string,string[]>();for(const t of sel){for(const x of t.ids)delete mapping[x];rep.set(t.id,t.replacement)}const custom=Object.fromEntries(Object.entries(cfg.analysis?.customize??{}).map(([k,v])=>[k,(v as string[]).flatMap(x=>rep.get(norm(x))??[x])]));out[id]=analyze(mapping,custom,Object.keys(input.expected));console.log(id)}writeFileSync(dir+'/review-native.json',JSON.stringify(out));