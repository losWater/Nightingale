...
const sel=JSON.parse(readFileSync(dir+'/wenjiao-selected.json','utf8'));const out:any={};for(const mode of ['base','use','joint']){const mapping={...cfg.form.mapping,龟:'a'};const rep=new Map<string,string[]>();for(const t of sel){for(const x of t.ids)delete mapping[x];rep.set(t.id,t.replacement)}if(mode!=='base')mapping['用']='a';if(mode==='joint'){delete mapping['角'];rep.set('角',[input.norm['⺈'],'用'])}const custom=Object.fromEntries(Object.entries(cfg.analysis?.customize??{}).map(([k,v])=>[k,(v as string[]).flatMap(x=>rep.get(norm(x))??[x])]));if(mode!=='base')delete custom['用'];if(mode==='joint')delete custom['角'];out[mode]=analyze(mapping,custom,Object.keys(input.expected));console.log(mode)}writeFileSync(dir+'/yong-yue-jiao-raw.json',JSON.stringify(out));