...
const sel=JSON.parse(readFileSync(dir+'/jian-selected.json','utf8'));const mapping={...cfg.form.mapping};const rep=new Map<string,string[]>();for(const t of sel){for(const id of t.ids)delete mapping[id];rep.set(t.id,t.replacement)}const custom=Object.fromEntries(Object.entries(cfg.analysis?.customize??{}).map(([k,v])=>[k,(v as string[]).flatMap(x=>rep.get(norm(x))??[x])]));const rows=analyze(mapping,custom,Object.keys(input.expected));writeFileSync(dir+'/jian-delete-raw.json',JSON.stringify(rows));console.log('完成');