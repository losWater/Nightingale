...
const sel=JSON.parse(readFileSync(dir+'/wenjiao-selected.json','utf8'));const out:any={};for(const mode of ['joint','variant']){const mapping={...cfg.form.mapping,龟:'a'};const rep=new Map<string,string[]>();for(const t of sel){for(const x of t.ids)delete mapping[x];rep.set(t.id,t.replacement)}if(mode!=='base')mapping['用']='a';if((mode==='joint'||mode==='variant')){delete mapping['角'];rep.set('角',[input.norm['⺈'],'用'])}if(mode==='variant')mapping['\ue08c']='a';const custom=Object.fromEntries(Object.entries(cfg.analysis?.customize??{}).map(([k,v])=>[k,(v as string[]).flatMap(x=>rep.get(norm(x))??[x])]));if(mode!=='base')delete custom['用'];if((mode==='joint'||mode==='variant'))delete custom['角'];out[mode]=analyze(mapping,custom,Object.keys(input.expected));console.log(mode)}writeFileSync(dir+'/yong-variant-raw.json',JSON.stringify(out));