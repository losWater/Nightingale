...
r=evaluate('MERGE-030','毛皮锚定',[('MERGE-030','毛皮锚定','毛','皮')])
r['说明']='当前140组基线，主根毛皮保留身份，连同附属根一起锚定；尚未裁定。'
r['根族']={name:[v['display'] for v in n['I']['inventory'] if G(v['id'])==G(n['rid'](name))] for name in ['毛','皮']}
(P/'毛皮锚定试算.json').write_text(json.dumps(r,ensure_ascii=False,indent=2),encoding='utf-8')
for k in ['根族','新增首根字对','消除首根字对','三码位净减少','新增完整重码','消除完整重码','新增完整详情','1.0三简风险','试算根组数','新增首根明细']:print(k,r[k])
md=['# 毛皮锚定试算','',r['说明'],'',str({k:r[k] for k in ['根族','新增首根字对','三码位净减少','新增完整重码','1.0三简风险','试算根组数']}),'','新增完整重码：'+str(r['新增完整详情']),'','新增首根冲突：'+str(r['新增首根明细'])]
(P/'毛皮锚定试算.md').write_text('\n'.join(md)+'\n',encoding='utf-8')
