...
const active=JSON.parse(readFileSync(dir+'/wenjiao-selected.json','utf8'));const targets=JSON.parse(readFileSync(dir+'/review-deletions.json','utf8'));const out:any={};for(const caseName of ['base','食','攴','支','攴支','辰','刃']){const key='er';const additions=caseName==='base'?[]:caseName==='攴支'?['攴','支']:[caseName];const mapping={...cfg.form.mapping,龟:'a',用:'a','\ue08c':'a'};const rep=new Map<string,string[]>();for(const t of [...active,...Array.from({length:14},(_,i)=>targets['DEL-'+String(i+3).padStart(3,'0')])]){for(const id of t.ids)delete mapping[id];rep.set(t.id,t.replacement)}delete mapping['角'];rep.set('角',[input.norm['⺈'],'用']);mapping['\ue05f']='a';if(key!=='base')mapping['午']='a';if(key==='ju'||key==='er'){delete mapping['击'];rep.set('击',key==='ju'?[input.norm['举字底'],'凵']:[input.norm['二'],'山']);}mapping['\ue021']='a';delete mapping['署'];rep.set('署',['罒','耂','日']);mapping['万']='a';delete mapping['佥'];rep.set('佥',targets['DEL-018'].replacement);for(const root of additions)mapping[root]='a';const custom=Object.fromEntries(Object.entries(cfg.analysis?.customize??{}).map(([k,v])=>[k,(v as string[]).flatMap(x=>rep.get(norm(x))??[x])]));if(key!=='base')delete custom['午'];if(key==='ju'||key==='er')custom['击']=rep.get('击');for(const root of additions)delete custom[root];delete custom['万'];delete custom['用'];delete custom['角'];out[caseName]=analyze(mapping,custom,Object.keys(input.expected));console.log(caseName)}writeFileSync(dir+'/candidate-additions-raw.json',JSON.stringify(out));