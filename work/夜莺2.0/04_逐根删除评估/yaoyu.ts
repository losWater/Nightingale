...
const selected=JSON.parse(readFileSync(dir+'/yaoyu-selected.json','utf8'));const output:any={};for(const ids of [[0],[1],[0,1]]){const mapping={...cfg.form.mapping};const rep=new Map<string,string[]>();for(const i of ids){const t=selected[i];for(const id of t.ids)delete mapping[id];rep.set(t.id,t.replacement)}const custom=Object.fromEntries(Object.entries(cfg.analysis?.customize??{}).map(([k,v])=>[k,(v as string[]).flatMap(x=>rep.get(norm(x))??[x])]));const rows=analyze(mapping,custom,Object.keys(input.expected));for(const [c,ts]of Object.entries(rows))if(ts.some(t=>rep.has(t)))throw new Error('残留 '+c);output[ids.join(',')]=rows;}writeFileSync(dir+'/yaoyu-raw.json',JSON.stringify(output));console.log('三种情形完成');