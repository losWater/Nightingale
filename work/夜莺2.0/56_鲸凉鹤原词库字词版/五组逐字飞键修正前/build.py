...
mod=runpy.run_path(str(W/'03_字音频率审计/rebuild.py'));fly=dict(mod['FLY']);fly[('道','jf')]='dc'
pairs=dict((b,a) for a,b in re.findall(r'derive/([a-z]{2});/([a-z]{2});/',(P/'WhaleCold_defs.yaml').read_text(encoding='utf-8')))
def restore(c,k):return fly.get((c,k),pairs.get(k,k))
...
summary={'词来源':str(source),'单字来源':str(base),'单字条目':sum(map(len,chars.values())),'非简词条目':sum(map(len,words.values())),'去除简词条目':len(excluded),'已知飞键还原条目':len(changes),'去重条目':len(duplicates),'长于四码条目':len(long),'字词同码位置':len(collisions),'规则':'保留所有四码及以上原词条（含长码和原词库特殊短语）；无简码单字优先，其次词，最后有任意1至3码的单字全码。同类顺序沿用来源顺序。','飞键限制':'另按WhaleCold_defs.yaml还原20组飞键；逐字映射：百be→bd、几jo→ji、一ei→yi、鹤eh→he、道jf→dc（用户确认作者特设飞键）。三字四码仅还原末字完整音节，其他缩写不猜测。未声称未知飞键已清零。','哈希':{str(f):hashlib.sha256(f.read_bytes()).hexdigest() for f in [source,base]}}
