...
def status(stage,**kwargs):
 ...
 page='<meta charset="utf-8"><meta http-equiv="refresh" content="30"><title>夜莺自动晋级赛进度</title><style>body{font:18px system-ui;margin:40px;background:#f4f7fb;color:#234}td,th{padding:10px;text-align:left}pre{white-space:pre-wrap}</style><h1>夜莺2.0 · 自动晋级赛</h1><p>第一轮256次 → 64个晋级 → 第二轮换语料 → 最终5个候选</p><table>'+''.join('<tr><th>'+html.escape(str(k))+'</th><td>'+html.escape(str(v))+'</td></tr>' for k,v in read(STATE).items())+'</table><p>进度每30秒自动刷新；正常运行无需打开对话。</p>'
...
def finish(finals,allrank):
 ...
 write(ROOT/'最终64排名.json',allrank);write(ROOT/'最终五名.json',finals)
...
def main():
 ...
 try:
  ...
  for group in range(1,SET['groups']+1):
   ...
   promoted.extend(chosen);write(ROOT/f'group_{group:02d}_ranking.json',ranked);write(ROOT/'晋级64名单_进行中.json',promoted)
  assert len(promoted)==64;write(ROOT/'晋级64名单.json',promoted)
  ...
  finish(finals,allrank);status('complete',first_round_completed=256,second_round_completed=64,finalists=[r['id'] for r in finals],report=str(ROOT/'最终候选报告.html'))
 except Exception:
