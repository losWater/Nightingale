...
def report():
 ds=[read(f) for f in sorted((P/'jobs').glob('*/round2.json'))]+[read(f) for f in sorted((P.parent/'20_手感权重五万步对照'/'jobs').glob('*/round2.json'))];ds.sort(key=lambda d:(d['card']['baseline'],d['card']['factor']));rows=[]
 for d in ds:
 ...
 page='<!doctype html><meta charset="utf-8"><title>手感权重微调对照</title><style>body{font:16px system-ui;background:#f3f6fa;color:#213047;margin:32px}table{border-collapse:collapse;background:white}td,th{padding:10px;border:1px solid #cbd5e1}p{line-height:1.8;max-width:1100px}</style><h1>手感权重微调对照</h1><p>本轮新增两个原始起点的×1.25、×1.5各一次，每次50000步；×1、×2复用上一轮结果。仅同比放大既有非零大跨排、小跨排和音形换手权重。三码、单字重码、字词碰撞、初终温度、根集、字频、简码规则均不变。当量和热力原目标为零，仍保留零；完整实战复测。用固定评分标准、同一第二轮一万句比较，不比较不同权重下的退火总分。原生随机流未固定，每格一次，只作趋势参考。</p>'+table(small)+'<h2>全部指标（可横向滚动）</h2><div style="overflow:auto">'+table(rows)+'</div>'
