...
fn 收集实际编码变化词(输出: &[编码信息]) -> Vec<usize> {
    输出
        .iter()
        .enumerate()
        .filter_map(|(index, info)| {
            (info.全码.有变化 || info.简码.有变化).then_some(index)
        })
        .collect()
}
...
impl 编码器 for 默认编码器 {
    ...
    fn 编码(
        ...
        if self.编码配置.简码配置列表.is_some()
            && !self.编码配置.简码配置列表.as_ref().unwrap().is_empty()
        {
            self.输出简码(输出);
        }
        // 夜莺2.0：同音的可用简码字让全；同类保持输入频率顺序。
        let mut order: Vec<usize> = (0..输出.len()).collect();
        order.sort_by_key(|&i| 输出[i].简码.原始编码 != 输出[i].全码.原始编码);
        let mut ranks: FxHashMap<u64, u8> = FxHashMap::default();
        for i in order {
            let row = &mut 输出[i];
            let raw = row.全码.原始编码;
            let rank = ranks.entry(raw).or_insert(0);
            let had_short = row.简码.原始编码 != raw;
            let length = self.编码配置.乘数列表.iter().position(|&n| raw < n).unwrap();
            let actual = self.编码配置.生成编码(raw, *rank, self.编码配置.乘数列表[length]);
            row.全码.原始编码候选位置 = *rank;
            row.全码.更新(actual, *rank);
            if !had_short {
                row.简码.原始编码候选位置 = *rank;
                row.简码.更新(actual, *rank);
            }
            *rank = rank.saturating_add(1);
        }
        // 必须在全码和简码都完成后采集。竞争者移动可能只改变另一个字的简码
        // 归属，而不改变该字全码；只看全码会让交叉碰撞增量缓存漏刷新。
        self.最近变化词 = 纯移动字根
            .is_some()
            .then(|| 收集实际编码变化词(输出));
    }
...
mod tests {
    use super::收集实际编码变化词;
    use crate::{编码信息, 部分编码信息};

    #[test]
    fn 变化集合同时包含全码变化和仅简码变化的字() {
        let mut 三字 = vec![
            编码信息 { 词长: 1, 频率: 3, 全码: 部分编码信息::default(), 简码: 部分编码信息::default() },
            编码信息 { 词长: 1, 频率: 2, 全码: 部分编码信息::default(), 简码: 部分编码信息::default() },
            编码信息 { 词长: 1, 频率: 1, 全码: 部分编码信息::default(), 简码: 部分编码信息::default() },
        ];
        三字[0].全码.有变化 = true;
        三字[1].简码.有变化 = true;
        assert_eq!(收集实际编码变化词(&三字), vec![0, 1]);
    }
}
