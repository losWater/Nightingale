...
const P='E:/夜莺2.0/work/夜莺2.0/38_固定200字当量专项试跑',W=P+'/..';
const paths=Object.fromEntries(['w0','w10','w40','w120'].map(x=>[x,P+'/'+x+'_五万步单字表.txt']));
const out=[];
...
const diff=n.逐字.map(x=>{const y=j.逐字.find(z=>z.字===x.字);if(!y)throw Error('missing '+x.字);return {字:x.字,w40码:x.码,w0码:y.码,w40当量:x.当量,w0当量:y.当量,加权差:(x.当量-y.当量)*x.字频/den,分项差:x.贡献.map((v,i)=>(v-y.贡献[i])*x.字频/den),音码一致:x.码.slice(0,2)===y.码.slice(0,2)}}).sort((a,b)=>b.加权差-a.加权差);
