...
base=rows(P.parent/'64_加入鲸凉鹤简词/夜莺2.0含简词字词表_普通格式.txt')
...
for s in (P/'参考模板/快符原表.txt').read_text(encoding='utf-8-sig').splitlines():
 ...
 if m:
  c,n,t=m.groups();n=int(n);quick.append((t,c));g[c].insert(n-1,t)
...
for c,ts in json.loads((P.parent/'64_加入鲸凉鹤简词/码位人工指定.json').read_text(encoding='utf-8')).items():assert {(t,c) for t in ts}<=wbset
wg=defaultdict(list)
...
try:
 for kind,label in [('light','轻量版'),('main','主力版')]:
  ...
  shutil.copy2(dest/'default.custom.yaml.example',dest/'default.custom.yaml')
