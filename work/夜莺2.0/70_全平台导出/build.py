...
assert len(rows)==len(set(rows))==213045
pointer=json.loads((W/'54_补删鹿旁保留羊南心四起点试跑/二简人工复核/当前裁定基线.json').read_text(encoding='utf-8-sig'))
baseline=Path(pointer['目录'])
single=[tuple(x.rsplit('\t',1)) for x in (baseline/'普通纯单字表.txt').read_text(encoding='utf-8-sig').splitlines() if x]
...
for kind,label in [('light','轻量版'),('main','主力版')]:
 ...
 for name in z.namelist():
  ...
  if name.endswith(('.yaml','.lua')) or name.endswith('.yaml.example'):
   s=data.decode('utf-8-sig').replace('yeying_','yeying20_').replace('1.0-rime-test1','2.0-20260914').replace('夜莺·','夜莺2.0·')
