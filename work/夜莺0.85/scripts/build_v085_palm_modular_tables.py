...
def main() -> None:
    source_path = PALM / "夜莺码v0.8.5电脑手心挂接.txt"
    source = read_palm(source_path)
    extension_chars = {r["字"] for r in read_tsv(TABLES / "夜莺码v0.8.5扩展字表.tsv")}
    single_pairs = set(read_plain(TABLES / "夜莺码v0.8.5单字版.txt"))
    ...
    ordinary_pairs = {
        (text, code) for text, code in read_plain(TABLES / "夜莺0.8.5字词表.txt")
    ...
    for code, rank, text in source:
        if (text, code, rank) in quick:
            name = "04_夜莺0.8.5_快符.txt"
        elif text in extension_chars and len(text) == 1:
            name = "05_夜莺0.8.5_扩展字.txt"
        elif is_short_word(text, code):
            name = "03_夜莺0.8.5_简词.txt"
        elif len(text) == 1:
            name = "01_夜莺0.8.5_核心单字.txt"
        elif (text, code) in ordinary_pairs:
            name = "02_夜莺0.8.5_普通词.txt"
        else:
