#!/usr/bin/env python3
"""Build the audited Nightingale 0.9.1 release and offline learning tools."""
...
def build_layout(config_path: Path, release: Path) -> tuple[dict, Path]:
    ...
    target = release / "夜莺码v0.9.1键位布局.yaml"
...
def build_practice(layout: dict, release: Path, roots_path: Path, resolve, aliases):
    ...
    data_path = release / "夜莺码v0.9.1字根练习.txt"
    data_path.write_text("\n".join(lines) + "\n", encoding="utf-8")

    page_template = TEMPLATE_DIR / "root_practice.html"
    page_path = release / "夜莺码v0.9.1字根练习器.html"
...
def build_lookup(single_path: Path, split_path: Path, release: Path, template: Path, fallback_font: Path):
    ...
    target = release / "夜莺码v0.9.1拆分查询.html"
...
def main() -> None:
    ...
    copy_exact(args.single, tables / "夜莺码v0.9.1单字版.txt")
    copies = [
        (args.single_sogou, "夜莺码v0.9.1无二字词版_搜狗.txt"),
        (args.single_sogou_quick, "夜莺码v0.9.1无二字词版_搜狗_含快符.txt"),
        (args.combined, "夜莺0.9.1字词表.txt"),
        (args.combined_sogou, "夜莺码v0.9.1挂接字词版_搜狗词库.txt"),
        (args.combined_sogou_quick, "夜莺码v0.9.1挂接字词版_搜狗词库_含快符.txt"),
        (args.decisions, "夜莺码v0.9.1字词裁决.tsv"),
        (args.single_decisions, "夜莺码v0.9.1单字裁决.tsv"),
        (args.irrational, "夜莺码v0.9.1无理码表.tsv"),
    ]
    ...
    reverse_code_count, reverse_entry_count = build_reverse_lookup(
        args.combined, tools / "夜莺码v0.9.1编码反查.html"
    )
