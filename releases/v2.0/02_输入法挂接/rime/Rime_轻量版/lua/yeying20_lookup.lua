...
function M.func(input, seg, env)
  ...
  if not env.lookup_data then env.lookup_data = require('yeying20_lookup_data') end
  ...
  for sound in pairs(sounds) do
    for _, row in ipairs(data.sounds[sound] or {}) do
      if not emitted[row[1]] and (not core_only or row[5] == 1) then
        ...
        if #codes > 0 then
          emitted[row[1]] = true
          yield(Candidate('yeying20_lookup', seg.start, seg._end, row[1], '〔'..row[2]..' · '..table.concat(codes,'/')..'〕'))
        end
      end
    end
  end
